...
function tight_upperbound(
    ...
    nta::Union{TighteningAlgorithm, Nothing} = Union{TighteningAlgorithm, Nothing}(),
    cutoff::Real = -Inf)
...
function relu(
    x::AbstractArray{T};
    #nta::Nullable{TighteningAlgorithm} = Nullable{TighteningAlgorithm}()
    nta::Union{TighteningAlgorithm, Nothing} = Union{TighteningAlgorithm, Nothing}())::Array{JuMP.AffExpr} where {T<:JuMPLinearType}
...
function masked_relu(
    ...
    #nta::Nullable{TighteningAlgorithm} = Nullable{TighteningAlgorithm}()
    nta::Union{TighteningAlgorithm, Nothing} = Union{TighteningAlgorithm, Nothing}())::Array{JuMP.AffExpr}
